...
class MyDataset(data.Dataset):
    def __init__(self, root_dir, train, size=(480, 640)):
        super().__init__()
        self.size = size
        print('size: ', size)
        self.train = train
    ...
    def __getitem__(self, idx):
        ...
        haze  = torch.Tensor(data[0])
        clear = torch.Tensor(data[1])
        
        # clear = tfs.CenterCrop((haze.size()[2], haze.size()[1]))(clear)
...
my_train_loader = DataLoader(dataset=MyDataset(path + '/train', train=True, size='whole img'), batch_size=BS, shuffle=True)
my_test_loader  = DataLoader(dataset=MyDataset(path + '/val', train=False, size='whole img'), batch_size=BS, shuffle=False)
